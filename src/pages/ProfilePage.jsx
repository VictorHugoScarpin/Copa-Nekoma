    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
      ...
      <div className="glass-card" style={{ padding: '16px 20px' }}>
        ...
        {[
          {
            icon: '🏆',
            label: 'Ranking Supercopa',
            color: 'var(--gold)',
            desc: 'A classificação geral. Soma todos os pontos da liga + bônus do torneio. É o ranking principal da Copa Nekoma.',
          },
          {
            icon: '⚽',
            label: 'Classificação Nekomão',
            color: 'var(--gold)',
            desc: 'Só conta os pontos da liga (palpites dos jogos). Sem bônus do torneio. Define quem entra no chaveamento.',
          },
          {
            icon: '🎌',
            label: 'Pontos Yuuto Kidou',
            color: '#c084fc',
            desc: 'Ranking exclusivo do torneio. Mostra os bônus acumulados por fase. Aparece em roxo no app.',
          },
        ].map((item, i, arr) => (
      ...
      <div className="glass-card" style={{ padding: '16px 20px' }}>
        ...
        <div style={{ fontSize: '11px', color: 'var(--text-3)', lineHeight: 1.8 }}>
          Os pontos do torneio aparecem em <strong style={{ color: '#c084fc' }}>roxo</strong> com o símbolo 🏆 sempre que aparecerem no app — tanto no ranking quanto no histórico de cada jogador.
        </div>
      </div>
        ...
        {subTabs.map(({ key, label }) => (
          <button
            ...
            style={{
              ...
              background: sub === key
                ? (key === 'torneio' ? 'rgba(168,85,247,0.2)' : 'rgba(255,255,255,0.1)')
                : 'transparent',
              color: sub === key
                ? (key === 'torneio' ? '#c084fc' : 'var(--text)')
                : 'var(--text-3)',
            }}
          >
            {label}
...
// ── Página principal com abas Perfil / Regras ─────────────────────────────────
export default function ProfilePage() {
  ...
  const pageTitles = { perfil: 'Perfil', regras: 'Regras' }
  ...
  return (
    <div className="page">
      ...
      <div style={{ display: 'flex', background: 'var(--surface)', borderRadius: 'var(--r-md)', padding: '4px', marginBottom: '20px', gap: '4px' }}>
        {[['perfil', '👤 Perfil'], ['regras', '📋 Regras']].map(([key, label]) => (
          <button
            key={key}
            onClick={() => setView(key)}
            style={{
